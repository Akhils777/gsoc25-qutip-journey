...
Following last week’s investigation into [Issue #47](https://github.com/qutip/qutip-qoc/issues/47), where the JOPT algorithm fails for open-system state transfer using the `TRACEDIFF` fidelity type, we revisited and reworked the proposed solution based on mentor feedback.
...
To address the issue, we’re now exploring a JAX-compatible alternative that uses raw JAX array operations instead of QuTiP methods. For example:
